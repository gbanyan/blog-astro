...
| `npm run check-delivery -- [origin]` | HTTP regression checks for Worker routing, cache headers, responsive covers, and 404s; defaults to a local Wrangler server at `http://127.0.0.1:4413`. |
...
- Static-assets-only Worker (`wrangler.jsonc`): `dist/` served at the edge,
  real 404s via `not_found_handling: "404-page"`; `drop-trailing-slash` matches
...
- Validate delivery locally on macOS: `npm run build`, then
  `npx wrangler dev --ip 127.0.0.1 --port 4413`, then `npm run check-delivery`.
  After deployment run `npm run check-delivery -- https://blog.gbanyan.net`.
  See [migration performance checks](docs/performance-migration.md) for evidence
  and the remaining optimization scope.
...
4. **Markdown images as-is** — inline Markdown images still use `/assets/*`.
   Covers and static card/list thumbnails now use responsive WebP variants
   generated by Astro; React-rendered list thumbnails still use originals.
